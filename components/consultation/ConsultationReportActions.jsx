...
import { SPECIALTIES, DOCTORS, TIME_SLOTS } from "@/lib/doctors";
import { getReportPreviewText } from "@/lib/report-format";
import MedicalReportPdfDocument from "@/components/report/MedicalReportPdfDocument";
import ReportPreview from "@/components/report/ReportPreview";

export default function ConsultationReportActions({
  transcript,
  report,
  isGeneratingReport,
  conversationActive,
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setSendFeedback({ type: "ok", text: `Sent to ${selectedDoctor.name}` });
    } catch (err) {
      setSendFeedback({ type: "err", text: err.message });
    } finally {
      if (fileInputRef.current) fileInputRef.current.value = "";
      setIsSending(false);
    }
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    try {
      ...
      setScheduleFeedback({ type: "ok", text: "Appointment requested!" });
      setScheduleDoctor(null);
    } catch (err) {
      setScheduleFeedback({ type: "err", text: err.message });
    } finally {
  ...
  return (
    <div className="flex-shrink-0 mt-3 pt-3 lg:pt-4 border-t border-slate-200">
      ...
      <div className="flex items-center gap-2 mb-2 lg:mb-3">
        ...
        {disabledReason && (
          <span className="text-[10px] lg:text-xs text-amber-600 ml-auto truncate">{disabledReason}</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4">
        ...
        <div className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50/80 to-white p-3 lg:p-5 flex flex-col lg:min-h-[172px]">
          <div className="flex items-start gap-2 lg:gap-3 mb-2 lg:mb-3">
            ...
            {hasReport && <CheckCircle2 className="h-4 w-4 lg:h-5 lg:w-5 text-green-500 flex-shrink-0" />}
          </div>
          ...
          <div className="flex gap-2 mt-auto">
            ...
            >
              {generating ? <Loader2 className="h-3.5 w-3.5 lg:h-4 lg:w-4 animate-spin" /> : <Sparkles className="h-3.5 w-3.5 lg:h-4 lg:w-4" />}
              <span className="hidden lg:inline">{generating ? "Generating…" : "Generate Report"}</span>
              ...
              >
                {({ loading }) => (
                  <>
                    ...
                    <span className="hidden lg:inline">{loading ? "Preparing…" : "Download PDF"}</span>
                    <span className="lg:hidden">{loading ? "…" : "PDF"}</span>
                  </>
                )}
              </PDFDownloadLink>
            ) : (
              <button type="button" disabled className="flex-1 h-9 lg:h-11 rounded-lg text-xs lg:text-sm font-medium flex items-center justify-center gap-1.5 border border-slate-100 bg-slate-50 text-slate-400 cursor-not-allowed">
                <Download className="h-3.5 w-3.5 lg:h-4 lg:w-4" />
        ...
        <div className="rounded-xl border border-emerald-100 bg-gradient-to-br from-emerald-50/80 to-white p-3 lg:p-5 flex flex-col lg:min-h-[172px]">
          <div className="flex items-start gap-2 lg:gap-3 mb-2 lg:mb-3">
            ...
            <div>
              <p className="text-xs lg:text-sm font-semibold text-slate-900">Specialist & Schedule</p>
              <p className="text-[10px] lg:text-xs text-slate-500 mt-0.5 hidden lg:block">
          ...
          <div className="flex-1 space-y-1.5 overflow-y-auto max-h-20 lg:max-h-28 pr-1">
            {filteredDoctors.length === 0 ? (
              <p className="text-[10px] text-slate-400 text-center py-2">No doctors in this specialty</p>
            ) : (
              filteredDoctors.map((doctor) => (
                ...
                >
                  <div className="flex items-center gap-1.5 min-w-0">
                    <img src={doctor.avatarUrl} alt="" className="h-7 w-7 lg:h-8 lg:w-8 rounded-full object-cover flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="text-[10px] lg:text-xs font-medium text-slate-800 truncate">{doctor.name}</p>
                      <p className="text-[9px] lg:text-[10px] text-slate-500 truncate">{doctor.speciality}</p>
                    </div>
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    >
                      ...
                      <span className="hidden lg:inline">{isSending && selectedDoctor?.id === doctor.id ? "…" : "Send"}</span>
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
          ...
          {(sendFeedback || scheduleFeedback) && (
            <p className={`text-[10px] lg:text-xs mt-1.5 truncate ${(sendFeedback || scheduleFeedback)?.type === "ok" ? "text-green-600" : "text-red-600"}`}>
              {(sendFeedback || scheduleFeedback)?.text}
            </p>
          )}
        </div>
      </div>
      ...
      <Dialog open={!!scheduleDoctor} onOpenChange={(open) => !open && setScheduleDoctor(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <form onSubmit={handleScheduleSubmit} className="space-y-3">
            <div>
              <Label htmlFor="appt-date" className="text-xs">Date</Label>
              <Input
                id="appt-date"
                type="date"
                required
            ...
            <Button type="submit" className="w-full" disabled={isScheduling || !scheduleDate || !scheduleTime}>
